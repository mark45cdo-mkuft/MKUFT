...
The repository mirror is split mechanically into eight ordered text parts for reliable cloning, recovery, and checksum verification. Those frozen parts are preservation objects and are not edited to track later live MKUFT changes. The version-DOI PDF on Zenodo remains the controlling object for exact pagination and visual layout.
...
The Voynich DOI paper previously remained discoverable through the research archive and DOI trail while the GitHub repository exposed only its later support module. The reader route above restores the standalone paper identity without replacing the exact Zenodo deposit.
