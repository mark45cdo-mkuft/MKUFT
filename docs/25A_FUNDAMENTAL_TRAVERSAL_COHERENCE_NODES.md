...
This module introduces a provisional cross-domain category. It does not establish a new physical force, universal node dynamics, consciousness in coherent nodes, or a domain-independent measurement scale.
...
1. **Convergence** — several relations, constraints, routes, or state transitions meet at the node.
2. **Anchoring** — the node preserves a stable reference against which neighbouring routes remain mutually compatible.
...
Enter through different legitimate routes and test whether they recover materially compatible versions of the candidate node and its constraints.

### 8.5 Perturbation and recovery

Disturb neighbouring routes, definitions, states, or constraints and measure whether the wider system returns toward the candidate signature.

### 8.6 Coalition ablation

Remove several apparent substitute nodes together to detect a distributed anchor network.

### 8.7 Stress and delay

Test whether an apparently dispensable node becomes load-bearing under transfer, contradiction, high demand, branch failure, delayed scoring, or repair.
...
Removal or fair substitution causes reproducible degradation beyond that explained by lost access, resources, or incident relations. The node's signature supports held-out function, transfer, convergence, or repair without concealed whole-system damage.

### Junction-only node

The node coordinates several relations, but its function is preserved by a fair relation-preserving reroute. It is useful but not anchor-specific under the tested conditions.

### Redundant anchor network

Individual node removal has little effect, but coalition removal produces material deformation. Anchoring is distributed rather than concentrated in one node.

### Latent or stress anchor

The node has little ordinary effect but becomes necessary under perturbation, transfer, contradiction, high load, or repair.

### Traversal-neutral node

Removal, rerouting, coalition testing, and stress testing produce no material effect within the declared boundary and measurement sensitivity.

### False coherence anchor

The node attracts, stabilises, or organises the system while reducing truth contact, calibration, agency, repairability, or long-horizon integrity. Local coherence is purchased through wider deformation.

### Unsupported anchor hypothesis
